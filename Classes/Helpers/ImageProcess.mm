...
//#import "OpenCV.framework/Headers/opencv2/opencv.hpp"
#import "opencv2.framework/Headers/opencv.hpp"
using namespace cv;

#define MAX_FEATURES 60
...
@property (nonatomic, assign) cv::Mat lastSmallImage;
@property (nonatomic, assign) cv::Mat currentSmallImage;
...
@synthesize lastSmallImage = _lastSmallImage;
@synthesize currentSmallImage = _currentSmallImage;
@synthesize reducedSize = _reducedSize;
@synthesize sizeMultiplier = _sizeMultiplier;
...
- (ImageProcess *) init {
    ...
    self.reducedSize = 100;
    self.sizeMultiplier = 4;
    return self;
}
...
- (int) fastFeatureDetection {
    self.maxFeatureNumber = 60;
    cv::resize(_lastImgMat, _lastSmallImage, cv::Size(self.reducedSize, self.reducedSize), 0, 0, CV_INTER_LINEAR);
    cv::resize(_currentImgMat, _currentSmallImage, cv::Size(self.reducedSize, self.reducedSize), 0, 0, CV_INTER_LINEAR);
    cv::FAST(self.lastSmallImage, fastFeaturePoints, self.threshold);
    //cv::FAST(self.lastImgMat, fastFeaturePoints, self.threshold);
    featureNum = fastFeaturePoints.size();
    NSLog(@"features %d", featureNum);
    ...
    for (int i = 0; i < featureNum; i++) {
        feature_pts1.row(i).col(0) = fastFeaturePoints[i].pt.y;
        feature_pts1.row(i).col(1) = fastFeaturePoints[i].pt.x;
...
- (CGPoint) featureTracking {
    cv::calcOpticalFlowPyrLK(self.lastSmallImage, self.currentSmallImage, feature_pts1, feature_pts2, status, err, winSize, maxLevel, criteria, derivLambda);
    ...
    if (self.trackedFeatureNum > 0) {
        ...
        NSLog(@"features123 %d", featureNum);
        NSDictionary *dict = @{@"features" : @(self.trackedFeatureNum),
                               @"total" : @(featureNum)
        };
        [[NSNotificationCenter defaultCenter]
        postNotificationName:@"TrackFeatures"
    ...
    NSLog(@"AvgX %f AvgY %f", avgX * 4, avgY * 4);
    return CGPointMake(avgX * self.sizeMultiplier, avgY * self.sizeMultiplier);
}
...
//    CGRect bounds = [[UIScreen mainScreen]bounds];
    NSLog(@"%@%f", @"ImageProcess-Y: ", y);
    // NSLog(@"%@%f", @"zoom Scale: ", currentScale);
